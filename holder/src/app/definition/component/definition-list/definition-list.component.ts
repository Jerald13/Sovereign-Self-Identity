...
export class DefinitionListComponent implements OnInit {
  ...
  enteredDefinitionID: any;
  onIDSearch() {
    this.agentService.getDefinitions(this.enteredDefinitionID)
      .pipe(
        map((definitionDetail: any) => {
          this.definitionDetail = definitionDetail;
          this.definitionIDs.push(definitionDetail?.id);
          console.log(this.enteredDefinitionID)
        })
      )
      .subscribe()
  }
